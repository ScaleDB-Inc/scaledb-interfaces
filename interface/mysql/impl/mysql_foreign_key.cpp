...
bool isSeparator(char c, char* separatorArray) {
...
char* MysqlForeignKey::getNextToken(char* token, char* cstr, bool ignore_seperator) {
	unsigned short i = 0;
	unsigned short j = 0;
	char* seperators=METAINFO_TOKEN_SEPARATORS;
	if(ignore_seperator) {seperators=METAINFO_TOKEN_SEPARATORS_NO_SPACE;}
	...
	if (cstr[i] == '`') {
		...
		while (!isSeparator(cstr[i],seperators) && (cstr[i] != '\0')) {
			token[j] = cstr[i];
			++i;
			++j;
		}
...
unsigned short MysqlForeignKey::setForeignKeyName(char* pForeignKeyName, bool ignore_seperator,
        char* pUserTableName, int fkNum) {
	unsigned short keyNameLen = 0;
	char keyName[METAINFO_MAX_IDENTIFIER_SIZE] = { 0 };

	getNextToken(keyName, pForeignKeyName,ignore_seperator);
...
unsigned short MysqlForeignKey::setKeyNumber(KEY* keyInfo, unsigned short numOfKeys, char* pOffset) {
	...
	if (i < numOfKeys) {
		...
		while (pOffset[0] != ')') {
			...
			getNextToken(colName, pOffset,false);
...
void MysqlForeignKey::setParentTableName(char* pTableName, bool ignore_sperator) {
	char tblName[METAINFO_MAX_IDENTIFIER_SIZE] = { 0 };

	getNextToken(tblName, pTableName,ignore_sperator);
	pParentTableName_ = SDBUtilDuplicateString(tblName);
}

void MysqlForeignKey::setParentColumnNames(char* pOffset,bool ignore_sperator) {
	for (unsigned short i = 0; i < numOfKeyFields; ++i) {
		char colName[METAINFO_MAX_IDENTIFIER_SIZE] = { 0 };

		getNextToken(colName, pOffset,ignore_sperator);
