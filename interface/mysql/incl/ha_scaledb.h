...
//			HA_STATS_RECORDS_IS_EXACT |
                             ...
                             enum ha_rkey_function find_flag)
   {
	...
	bool generateAnalyticsString(List<Item>* fields, Item* having);
	...
	// create a user table.
	int parseTableOptions( THD *thd, TABLE * table, HA_CREATE_INFO *create_info,
						   bool& streamTable, bool& dimensionTable, unsigned long long&  dimensionSize,
						   char** rangekey, char** streamTime,  int* streamStart, unsigned short* streamInterval, bool create_from_like );
	...
	int add_columns_to_table( THD* thd, bool isAlter, TABLE *table_arg, unsigned short ddlFlag,unsigned short tableCharSet,
							  char* pSystemRangekey, char* pUserRangeTime, int userTimeRangeStart, unsigned short typeRangeInterval, int& number_streaming_keys,
							  int& number_streaming_attributes, bool dimensionTable, SdbDynamicArray* fkInfoArray, bool create_from_like );
...
//			case SQLCOM_TRUNCATE:
	...
	inline bool isRangeRead( TABLE_LIST* pTableList , bool* is_streaming, bool* multiple_ranges)
	{
		...
		*is_streaming					= SDBIsStreamingTable( dbId, tableId );
		...
		if(number_ranges>1) 
		{
			//mariadb has determined that multiple ranges are required for this quesry which analytics donot support  
			//eg 
			//
			// SELECT      sum(uSMALLINT) FROM test.streaming_scaledb WHERE user_time<>2 ;
			//
			//which is broken into 2 queries   select ... <2 and select... >2
			//
			//if we dont fail the query then the wrong result is returned.
			//
	...
	bool generateProjectionString(List<Item> *list,  Item* is_null_orderby, unsigned char* projection_buf, int* len, bool projection_optimization); 
	bool extractWhereFields(COND* cond, char* buf, int& pos, int& n, bool& contains_analytic);
...
private:
	...
	inline void setSqlCommand( int sqlCommand )
	{
		sqlCommand_				= sqlCommand;
	}

	inline void initStatementVariables( int sqlCommand )
	{
		// Initialize variables for executing a statement
		isIndexedQuery_			=
	...
	inline static bool compareTemporalFieldOperands( Item* pFieldItem, unsigned char* pFieldData, unsigned short dbId, unsigned short tableId,
													 Item* pComperandFieldItem, unsigned char* pComperandFieldData )
	{
		int					typeComperandField	= ( ( ( Item_field* ) pComperandFieldItem )->field )->type();

		switch ( typeComperandField )
		{
			case MYSQL_TYPE_TIMESTAMP:
			case MYSQL_TYPE_TIME:
			case MYSQL_TYPE_DATE:
		...
		int					typeField			= ( ( ( Item_field* ) pFieldItem )->field )->type();
		unsigned short		dbIdComperand		= *( unsigned short* )( pComperandFieldData + ROW_DATA_OFFSET_DATABASE_NUMBER );
		unsigned short		tableIdComperand	= *( unsigned short* )( pComperandFieldData + ROW_DATA_OFFSET_TABLE_NUMBER );

		if ( ( dbIdComperand == dbId )		   && ( tableIdComperand == tableId ) && ( typeComperandField == typeField ) )
		{
			return true;
		}
	...
	inline static int convertTimeStructToTimeType( MYSQL_TIME* pMyTime )
	{
		...
		if ( pMyTime->second_part )
		{
			// Round the time value since we don't handle the fractional seconds component
			if ( pMyTime->second_part		   >= 500000 )
			{
				// Round up the time value
				//	Note that this may result in an invalid value, but this is OK for comparison purposes
				( pMyTime->second )++;

				if ( pMyTime->second		   == 60 )
				{
					pMyTime->second				= 0;

					( pMyTime->minute )++;

					if ( pMyTime->minute	   == 60 )
					{
						pMyTime->minute			= 0;

						( pMyTime->hour )++;

						if ( pMyTime->day	   && ( pMyTime->hour == 24 ) )
						{
							pMyTime->hour		= 0;
	...
	inline static bool compareDecimalFieldOperands( Item* pFieldItem, unsigned char* pFieldData, unsigned short dbId, unsigned short tableId,
													Item* pComperandFieldItem, unsigned char* pComperandFieldData )
	{
		int					typeComperandField	= ( ( ( Item_field* ) pComperandFieldItem )->field )->type();
		...
		Item_field*			pComperandField		= ( Item_field* ) pComperandFieldItem;
		int					iPrecision			= pField->decimal_precision();
		int					iScale				= pField->decimals;
		int					iComperandPrecision	= pComperandField->decimal_precision();
		int					iComperandScale		= pComperandField->decimals;
		unsigned short		dbIdComperand		= *( unsigned short* )( pComperandFieldData + ROW_DATA_OFFSET_DATABASE_NUMBER );
		unsigned short		tableIdComperand	= *( unsigned short* )( pComperandFieldData + ROW_DATA_OFFSET_TABLE_NUMBER );

		if ( ( dbIdComperand == dbId )		   && ( tableIdComperand == tableId ) && ( iComperandPrecision == iPrecision ) && ( iComperandScale == iScale ) )
		{
			return true;
		}
	...
	inline static bool compareIntegerFieldOperands( Item* pFieldItem, unsigned char* pFieldData, unsigned short dbId, unsigned short tableId,
													Item* pComperandFieldItem, unsigned char* pComperandFieldData )
	{
		int					comperandFieldType	= ( ( ( Item_field* ) pComperandFieldItem )->field )->type();

		switch ( comperandFieldType )
		{
			case MYSQL_TYPE_TINY:
			case MYSQL_TYPE_SHORT:
			case MYSQL_TYPE_INT24:
		...
		unsigned short		dbIdComperand		= *( unsigned short* )( pComperandFieldData + ROW_DATA_OFFSET_DATABASE_NUMBER );
		unsigned short		tableIdComperand	= *( unsigned short* )( pComperandFieldData + ROW_DATA_OFFSET_TABLE_NUMBER );
		unsigned char		typeComperandField	= *( unsigned char*  )( pComperandFieldData + ROW_DATA_OFFSET_ROW_TYPE );
		unsigned char		typeField			= *( unsigned char*  )( pFieldData			+ ROW_DATA_OFFSET_ROW_TYPE );
	...
	inline static bool compareFloatFieldOperands( Item* pFieldItem, unsigned char* pFieldData, unsigned short dbId, unsigned short tableId,
												  Item* pComperandFieldItem, unsigned char* pComperandFieldData )
	{
		int					comperandFieldType	= ( ( ( Item_field* ) pComperandFieldItem )->field )->type();
	...
	inline static bool compareStringFieldOperands( Item* pFieldItem, unsigned char* pFieldData, unsigned short dbId, unsigned short tableId,
												   Item* pComperandFieldItem, unsigned char* pComperandFieldData )
	{
		int					comperandFieldType	= ( ( ( Item_field* ) pComperandFieldItem )->field )->type();
	...
	inline bool getRangeKeyDefaultValue( TABLE* table_arg, Field* pField, enum_field_types typeField, unsigned short typeRangeInterval, long long& valueDefault )
	{
		...
		// Get the default value of a range key column
		my_bitmap_map*		pSavedBitmap	= dbug_tmp_use_all_columns( table_arg, table_arg->read_set );

		pField->move_field_offset( table_arg->s->default_values - table_arg->record[ 0 ] );

		switch ( typeField )
		{
			case MYSQL_TYPE_LONG:
				valueDefault				= pField->val_int();
				break;

			case MYSQL_TYPE_TIMESTAMP:
			{
				const uint	maxTimeStringSize( 65535 );	// Maximum length of a time string
				char		chBuf[ maxTimeStringSize ];
				THD*		pMysqlThd		= ha_thd();
				String		stringValue( chBuf, maxTimeStringSize, pMysqlThd->charset() );
				MYSQL_TIME	myTime;
				uint		uiError;

				pField->val_str( &stringValue );

				// Convert the time string to a time value
				if ( !( convertStringToTime( pMysqlThd, stringValue.ptr(), maxTimeStringSize, MYSQL_TYPE_TIMESTAMP, &myTime ) ) )
				{
					// Unable to convert the time string to a valie time value
					SDBSetErrorMessage( sdbUserId_, INVALID_STREAMING_OPERATION, "- Invalid temporal value as a range key default value." );
					return false;
				}

				// Convert the time value  to a timestamp
				valueDefault				= TIME_to_timestamp( pMysqlThd, &myTime, &uiError );

				if ( uiError )
				{
					// Timestamp conversion error
					//	Try to convert an over limit time value to a valid timestamp that expresses the same limit
					if ( !( convertTimeValueBeyondTimestampLimit( &myTime, valueDefault ) ) )
					{
						// Unable to convert the time value to a valid timestamp
						SDBSetErrorMessage( sdbUserId_, INVALID_STREAMING_OPERATION, "- Invalid temporal value as a range key default value." );
						return false;
			...
			default:
				SDBTerminateEngine( 0, "Invalid range key field type", __FILE__, __LINE__ );
		}

		pField->move_field_offset( table_arg->record[ 0 ] - table_arg->s->default_values );
		dbug_tmp_restore_column_map( table_arg->read_set, pSavedBitmap );
